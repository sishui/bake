...
COMMENT
ON COLUMN "public"."test_all_types"."json_val" IS 'This is a JSON column
...
VALUES (2, 1, 1001, 10, EXTRACT(EPOCH FROM NOW())::BIGINT, EXTRACT(EPOCH FROM NOW())::BIGINT),
...
CREATE INDEX idx_start_end
    ON users USING BTREE (start_at, end_at);

CREATE INDEX idx_seq
    ON users USING BTREE (seq);
...
INSERT INTO users (name, email, start_at, end_at, seq)
VALUES ('Alice', 'alice@example.com', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, 1),
       ('Bob', 'bob@example.com', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, 2);
...
(
    ...
    CONSTRAINT test_index_seq_key UNIQUE (seq)
);
...
(
    ...
    CONSTRAINT test_multi_index_start_end_key
